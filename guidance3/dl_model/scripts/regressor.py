...
from guidance3.dl_model.data_processing.group_aware_zscore_scaler import GroupAwareScalerZ
from guidance3.dl_model.batching.batch_generator import BatchGenerator
import pandas as pd
import numpy as np
...
def _balanced_sample_by_code1_and_taxa(df):
    ...
    frequent_codes = code_counts[code_counts >= 1600].index #TODO - used only to get balanced set per taxa with 1600 alt MSAs for each code, change number if needed
    ...
    selected_codes_set = set(frequent_codes_balanced_per_taxa) #TODO the set has names of the MSAs but not per code
...
def _balanced_sample_1600(df):
    code_counts = df['code1'].value_counts()
    frequent_codes = code_counts[code_counts >= 1604].index #TODO change number if needed
...
def _assign_true_score_name(predicted_measure: str) -> str:
    if predicted_measure == 'msa_distance':
        ...
        true_score_name = 'class_label'
            ...
            raise ValueError(f"Invalid predicted_measure: {predicted_measure}, the allowed options are 'msa_distance', 'tree_distance' or 'class_label'\n")
...
def _zscore_targets_per_group(y_true: pd.Series, group_codes: pd.Series) -> pd.Series:
    ...
    for group_val, group_df in df.groupby("group"):
        ...
        scaled_series.loc[group_df.index] = scaled_vals # assign scaled values to same indices
...
class Regressor:
    ...
    mode: 1 is all features (default), 2 is all except SoP features, 3 is chosen (hard-coded in constants) list of features
    ...
    def __init__(self, features_file: str, test_size: float, mode: int = 1,
                 ...
                 true_score_name:
                 Literal['ssp_from_true', 'dseq_from_true', 'dpos_from_true', 'RF_phangorn_norm'] = 'dseq_from_true',
        ...
        if scaler_type_features not in {'standard', 'rank', 'zscore'}:
            raise ValueError(f"Invalid scaler_type_features: {scaler_type_features}, "
                             f"the allowed options are 'standard' , 'rank' or 'zscore'\n")
        if scaler_type_labels not in {'standard', 'rank', 'zscore'}:
            raise ValueError(f"Invalid scaler_type_labels: {scaler_type_labels}, "
                             f"the allowed options are 'standard' , 'rank' or 'zscore'\n")
        ...
        if true_score_name not in {'ssp_from_true', 'dseq_from_true', 'dpos_from_true', 'RF_phangorn_norm'}:
            raise ValueError(f"Invalid true_score_name: {true_score_name}, "
                             f"the allowed options are 'ssp_from_true', 'dseq_from_true', 'dpos_from_true', 'RF_phangorn_norm'\n")

        df = _read_features_into_df(self.features_file)

        # if self.with_embeddings:  # add LLM embeddings
        ...
        # df = df[df['aligner'] != 'true']  # remove all True MSAs
        ...
        if self.deduplicated:
            df = df.drop_duplicates(subset=[col for col in df.columns if col != 'code'])
            summary = _summarize_data(df)

        else: # not dropping all duplicates but only very problematic codes
        ...
        if self.true_score_name != 'RF_phangorn_norm':
            if self.empirical:
                ...
                #just to get balanced set per taxa with 1600 alt MSAs for each code
            summary = _summarize_data(df)
    ...
    def _save_scaled(self, i):
        ...
        # x_train_scaled_to_save.columns = self.X_train.columns #uncomment
        ...
        # x_test_scaled_to_save.columns = self.X_test.columns #uncomment
    ...
    def deep_learning(self, epochs: int = 50, batch_size: int = 32, validation_split: float = 0.2, verbose: int = 1,
        ...
        def lambda_rank_loss(y_true, y_pred, eps=1e-10):
            ...
            # pairwise labels: 1 if i < j (true_i < true_j means i is better)
            S = tf.cast(diff_true < 0, tf.float32)
