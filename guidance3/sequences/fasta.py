...
from guidance3.trees.phylo import fix_mafft_rough_tree
import tarfile
import glob
...
#@timeit
def add_original_seq_names_to_the_MSA(config):
    config.Alignment_File_With_Names = config.Alignment_File + ".With_Names"
    ans = codes2name_fasta_from1(f"{config.WorkingDir}{config.Alignment_File}",
                                 f"{config.WorkingDir}{config.code_fileName}",
                                 f"{config.WorkingDir}{config.Alignment_File_With_Names}")
    if ans[0] != "OK":
        exit_on_error("sys_error",
                      f"Guidance::codes2nameFastaFrom1: Guidance::codes2nameFastaFrom1({config.WorkingDir}{config.Alignment_File},{config.WorkingDir}{config.code_fileName},{config.WorkingDir}{config.Alignment_File_With_Names}) failed:" +
                      ''.join(ans) + "\n", config)


def extract_seq_from_MSA(in_msa, seq_file, config):
    line_num = 0
    try:
...
def align(config):
    ...
    config.Alignment_File = f"{config.dataset}.{config.MSA_Program}.aln"
    config.Core_Alignment_File = f"{config.dataset}.{config.MSA_Program}.CORE.aln"
    ...
    if config.MSA_Program == "MAFFT":
        ...
        if 'addfragments' in config.align_param:
            aln_param_for_core_msa = config.align_param
            ...
            if config.Seq_Type in ["AminoAcids", "Codons"]:
                cmd = f"{config.mafft_prog} {aln_param_for_core_msa} --amino --quiet --thread {config.proc_num} {config.WorkingDir}{config.codded_seq_fileName} > {config.WorkingDir}{config.Core_Alignment_File}"
            elif config.Seq_Type == "Nucleotides":
                cmd = f"{config.mafft_prog} {aln_param_for_core_msa} --nuc --quiet --thread {config.proc_num} {config.WorkingDir}{config.codded_seq_fileName} > {config.WorkingDir}{config.Core_Alignment_File}"
            ...
            if not os.path.exists(
                    ...
                    f"{config.WorkingDir}{config.Core_Alignment_File}") == 0:
                exit_on_error("sys_error",
                              f"Align: '{config.WorkingDir}{config.Core_Alignment_File}' is empty/does not exist", config)

            # create the full alignment with fragments
            if config.Seq_Type in ["AminoAcids", "Codons"]:
                cmd = f"{config.mafft_prog} {config.align_param} --amino --quiet {config.WorkingDir}{config.Core_Alignment_File} > {config.WorkingDir}{config.Alignment_File}"
            elif config.Seq_Type == "Nucleotides":
                cmd = f"{config.mafft_prog} {config.align_param} --nuc --quiet {config.WorkingDir}{config.Core_Alignment_File} > {config.WorkingDir}{config.Alignment_File}"

        else:
            if config.Align_Order == "aligned" and 'reorder' not in config.align_param:
                config.align_param += " --reorder"

            if config.Seq_Type in ["AminoAcids", "Codons"]:
                cmd = f"{config.mafft_prog} {config.align_param} --amino --quiet {config.WorkingDir}{config.codded_seq_fileName} > {config.WorkingDir}{config.Alignment_File}"
            elif config.Seq_Type == "Nucleotides":
                cmd = f"{config.mafft_prog} {config.align_param} --nuc --quiet {config.WorkingDir}{config.codded_seq_fileName} > {config.WorkingDir}{config.Alignment_File}"
        ...
        cmd = ""
        if config.Seq_Type in ["AminoAcids", "Codons"]:
            cmd = f"{config.mafft_prog} --localpair --maxiterate 1000 --amino --quiet {config.WorkingDir}{config.codded_seq_fileName} > {config.WorkingDir}{config.Alignment_File}"
        elif config.Seq_Type == "Nucleotides":
            cmd = f"{config.mafft_prog} --localpair --maxiterate 1000 --nuc --quiet {config.WorkingDir}{config.codded_seq_fileName} > {config.WorkingDir}{config.Alignment_File}"
    ...
    if config.MSA_Program == "PRANK":
        ...
        prank_help = subprocess.getoutput(config.prank_prog)
        ...
        if config.Seq_Type in ["AminoAcids", "Nucleotides", "Codons"]:

            if PRANK_VERSION == 0:

                cmd = f"{config.prank_prog} {config.align_param} -quiet -d={config.WorkingDir}{config.codded_seq_fileName} -o={config.WorkingDir}{config.Alignment_File} -noxml > {config.WorkingDir}{config.Alignment_File}.std"

            else:  # version >=120626

                cmd = f"{config.prank_prog} {config.align_param} -quiet -d={config.WorkingDir}{config.codded_seq_fileName} -o={config.WorkingDir}{config.Alignment_File} > {config.WorkingDir}{config.Alignment_File}.std"
        ...
        if PRANK_VERSION == "121218":  # The output file now named best.fas
            ...
            shutil.copy(f"{config.WorkingDir}{config.Alignment_File}.best.fas",
                        f"{config.WorkingDir}{config.Alignment_File}")
    ...
    if config.MSA_Program == "CLUSTALO":
        ...
        if config.Seq_Type in ["AminoAcids", "Codons"]:
            cmd = f"{config.clustalw_prog} --infile={config.WorkingDir}{config.codded_seq_fileName} --outfile={config.WorkingDir}{config.Alignment_File} --seqtype=Protein > {config.WorkingDir}{config.Alignment_File}.std"
        elif config.Seq_Type == "Nucleotides":
            cmd = f"{config.clustalw_prog} --infile={config.WorkingDir}{config.codded_seq_fileName} --outfile={config.WorkingDir}{config.Alignment_File} --seqtype=DNA > {config.WorkingDir}{config.Alignment_File}.std"
        ...
        # shutil.move(f"{config.WorkingDir}{config.Alignment_File}",
        #             f"{config.WorkingDir}{config.Alignment_File}.orig")
        # shutil.move(f"{config.WorkingDir}{config.Alignment_File}.fs",
        #             f"{config.WorkingDir}{config.Alignment_File}")

        if config.Align_Order == "as_input":
            with open(config.OutLogFile, "a") as log_file:
                log_file.write(f"MSA_parser.sort_alignment({config.WorkingDir}{config.Alignment_File}, fasta);\n")
    ...
    if config.MSA_Program == "MUSCLE":
        ...
        if config.Seq_Type in ["AminoAcids", "Codons"]:
            cmd = f"{config.muscle_prog} -quiet -in {config.WorkingDir}{config.codded_seq_fileName} -out {config.WorkingDir}{config.Alignment_File} -seqtype protein > {config.WorkingDir}{config.Alignment_File}.std"
        elif config.Seq_Type == "Nucleotides":
            cmd = f"{config.muscle_prog} -quiet -in {config.WorkingDir}{config.codded_seq_fileName} -out {config.WorkingDir}{config.Alignment_File} -seqtype dna > {config.WorkingDir}{config.Alignment_File}.std"
    ...
    if not os.path.exists(f"{config.WorkingDir}{config.Alignment_File}") or os.path.getsize(
            f"{config.WorkingDir}{config.Alignment_File}") == 0:
        exit_on_error("sys_error",
                      f"Align: '{config.WorkingDir}{config.Alignment_File}' is empty/does not exist\n", config)


def codon_alignment_to_aminoacids_alignment(codon_aln, AA_aln, codon_table, xcodon_MSAfile, config):
    try:
            ...
            for line in codon_file:
                ...
                if not line.startswith(">") and line != "":
                    ...
                    if seq and seq_name:
                        ...
                        #                                                        f"{config.WorkingDir}{xcodon_MSAfile}")
...
def create_tar_archives(config):
    if config.PROGRAM == "GUIDANCE3":
        alt_msas_dir = os.path.join(config.GUIDANCE3_MSAs_Dir, 'wNames')
        os.makedirs(alt_msas_dir, exist_ok=True)
        guidance3_msas_name_pattern = os.path.join(config.GUIDANCE3_MSAs_Dir, '*.fasta')

        for alt_msa in glob.glob(guidance3_msas_name_pattern):
            alt_msa_no_path = os.path.basename(alt_msa)

            if config.Seq_Type == "Codons":
                DNA_AA_cp = {key: config.DNA_AA[key][:] for key in config.DNA_AA}
                AA_to_DNA_aligned(alt_msa, f"{alt_msa}.AA", DNA_AA_cp)
                codes2name_fasta_from1(f"{alt_msa}.AA", os.path.join(config.WorkingDir, config.code_fileName),
                                     os.path.join(alt_msas_dir, alt_msa_no_path))
                os.remove(f"{alt_msa}.AA")
            else:
                codes2name_fasta_from1(alt_msa, os.path.join(config.WorkingDir, config.code_fileName),
                                     os.path.join(alt_msas_dir, alt_msa_no_path))

        # Create a tar.gz file for alternative MSAs
        with tarfile.open(os.path.join(config.WorkingDir, f"{config.Output_Prefix}_AlternativeMSA.tar.gz"),
                          "w:gz") as tar:
        ...
        alt_msa_dest = os.path.join(config.WorkingDir, f"{config.Output_Prefix}_AlternativeMSA")
        ...
        config.List_Of_Alternative_MSAs = os.path.join(config.WorkingDir, "List_Of_Default_and_AltMSAs.txt")
        with open(config.List_Of_Alternative_MSAs, "w") as _list_f:
            default_with_names = os.path.join(config.WorkingDir, config.Alignment_File_With_Names)
            alt_fasta_list = sorted(glob.glob(os.path.join(alt_msa_dest, "*.fasta")))
            _list_f.write("\n".join([default_with_names] + alt_fasta_list) + "\n")

    if config.PROGRAM in ["GUIDANCE", "GUIDANCE3"]:
        # Tar and remove the BP dir
        cmd = f"cd {config.WorkingDir}; tar -czf {config.Output_Prefix}_BP_Dir.tar.gz ./BP"
        os.system(cmd)
        shutil.rmtree(config.BootStrap_Dir, ignore_errors=True)

    if config.PROGRAM == "HoT":
        # Tar and remove the HoT MSAs dir
        cmd = f"cd {config.WorkingDir}; tar -czf {config.dataset}.{config.MSA_Program}_HoT_Dir.tar.gz ./{config.HoT_MSAs_Dir} ./{config.dataset}_cos_{config.HoT_MSA_Program}"
        os.system(cmd)
        shutil.rmtree(os.path.join(config.WorkingDir, f"{config.dataset}_cos_{config.HoT_MSA_Program}"), ignore_errors = True)
...
def select_best_msa(config):
    ...
    run_id = os.path.basename(os.path.normpath(config.WorkingDir))

    alt_msas_dir = os.path.join(config.WorkingDir, f"{config.Output_Prefix}_AlternativeMSA")
    default_msa = os.path.join(config.WorkingDir, config.Alignment_File_With_Names)
    ...
    features_input_dir = os.path.join(config.WorkingDir, "features_input")
    ...
    features_cfg = {
        ...
        "output_file_dir_path": config.WorkingDir,
        "is_unified_file": True,
        "matrix_dir_path": FEATURES_EXTRACTION_MATRIX_DIR,
    }

    config_path = os.path.join(config.WorkingDir, "config_features.json")
    ...
    pred_out_dir = os.path.join(config.WorkingDir, "best_msa_pred")
