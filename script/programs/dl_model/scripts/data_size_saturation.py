...
class RegressorPortion:
    ...
    def deep_learning(self, epochs=50, batch_size=16, validation_split=0.2, verbose=1, learning_rate=0.01, i=0, undersampling = False):
        ...
        plt.legend()
        plt.savefig(fname=f'../../example_data/predictions/loss_graph_{self.portion}_mode{self.mode}_{self.predicted_measure}.png', format='png')
        plt.show()
        plt.close()

        # visualize model architecture
        plot_model(model, to_file=f'../../example_data/predictions/model_architecture_{self.portion}_mode{self.mode}_{self.predicted_measure}.png', show_shapes=True, show_layer_names=True,
                   show_layer_activations=True)
        model.save(f'../../example_data/predictions/regressor_model_{self.portion}_mode{self.mode}_{self.predicted_measure}.keras')
        # Save the model architecture as a Dot file
        plot_model(model, to_file='../../example_data/predictions/model_architecture.dot', show_shapes=True, show_layer_names=True)
        ...
        df_res.to_csv(f'../../example_data/predictions/prediction_DL_{self.portion}_mode{self.mode}_{self.predicted_measure}.csv', index=False)
    ...
    def plot_results(self, model_name: Literal["svr", "rf", "knn-r", "gbr", "dl"], mse: float, i: int) -> None:
        ...
        plt.grid(True)
        plt.savefig(fname=f'../../example_data/predictions/regression_results_{self.portion}_mode{self.mode}_{self.predicted_measure}.png', format='png')
        plt.show()
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(fname=f'../../example_data/predictions/regression_results_{self.portion}_mode{self.mode}_{self.predicted_measure}2.png', format='png')
        plt.show()
        plt.close()
...
if __name__ == '__main__':
    ...
    for portion in [0.1,0.2,0.4]:
        regressor = RegressorPortion("../input/ortho12_distant_features_121125.csv", 0.2, mode=1,
