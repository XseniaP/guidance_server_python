...
" unless (@ARGV >= 1);
my $OVERHEAD_TIME=5; # time added to base MSA time to account for system overhead.
if ($ARGV[0] =~ m/^-/) {
	...
	if ($FORM{PROGRAM} eq "GUIDANCE2")
	{
		...
		$credits_URL="http://guidance.tau.ac.il/ver2/credits.php";
	}
	else
	{
		...
		$credits_URL="http://guidance.tau.ac.il/credits.html";
	}
...
# 	$msa_set_score_prog=Guidance::MSA_SET_SCORE;
	...
	# assume the installation was made with conda env and relative to the guidance.pl code to be more similar to the standalone version
	$mafft_prog = "mafft";
	$prank_prog = "prank";
	$clustalw_prog = "clustalw";
	$muscle_prog = "muscle";
	...
	{ # all was: http://guidance.tau.ac.il/ver2/
		$overview_URL="http://guidance.tau.ac.il/overview.php";
		$gallery_URL="http://guidance.tau.ac.il/Gallery.php";
		$home_URL="http://guidance.tau.ac.il/";
		$credits_URL="http://guidance.tau.ac.il/credits.php";
	}
	else
	{
		$overview_URL="http://guidance.tau.ac.il/overview.html";
...
# if ($isServer==1)
# {
# 	# For server mode use the none public params file...
# 	use lib "/bioseq/bioSequence_scripts_and_constants";
# 	use GENERAL_CONSTANTS;
# }
...
if ((-e "$VARS{WorkingDir}$VARS{Alignment_File}") and (-s "$VARS{WorkingDir}$VARS{Alignment_File}" > 0))
  {
	  ...
	  if (($FORM{Seq_Type} eq "Nucleotides")) # ?? ($FORM{Seq_Type} eq "Codons") ??
	  {
		  ...
		  else  # GUIDANCE2 or HoT
		  {
			  if ($FORM{MSA_Program} eq "MAFFT")
			  {
				  convert_fs_to_upper_case("$VARS{WorkingDir}$VARS{Alignment_File}.WithCodesName"); # HoT and GUIDANCE2 ALWAYS OUTPUT NUC MSA IN LOWER CASE
			  }
		  }		
	  }
...
if ($FORM{Seq_Type} eq "Codons")
{
	...
	if (($FORM{PROGRAM} eq "GUIDANCE2") or ($FORM{PROGRAM} eq "GUIDANCE3") or ($FORM{PROGRAM} eq "GUIDANCE3_HOT")) # TO DO: check if conversion is needed in GUIDANCE3 without HoT, did not do it for non codons seq
	{
		...
		@ans=codonAlign::translate_DNA_to_AA("$VARS{WorkingDir}$VARS{codded_seq_fileName_Codons}","$VARS{WorkingDir}$VARS{codded_seq_fileName}",$FORM{CodonTable},"$VARS{WorkingDir}$VARS{TranslateErrors}","$VARS{WorkingDir}$VARS{output_page}",\%DNA_AA,$VARS{run_url},"","seqNum");
	}
	else
	{
		...
		@ans=codonAlign::translate_DNA_to_AA("$VARS{WorkingDir}$VARS{codded_seq_fileName_Codons}","$VARS{WorkingDir}$VARS{codded_seq_fileName}",$FORM{CodonTable},"$VARS{WorkingDir}$VARS{TranslateErrors}","$VARS{WorkingDir}$VARS{output_page}",\%DNA_AA, $VARS{run_url});
	}
	if ($ans[0] ne "ok")
	{
		if ($ans[0] eq "user")
...
#	print OUTPUT "<h4><font face=Verdana><u>Running Messages:</u></h4></font>\n";
	print OUTPUT "<h4><font face=Verdana><u>Running Progress:</u></h4></font>\n";
	&Print_Initial_Running_Progress();
    # JS
	#print OUTPUT "<?php\n\tinclude ('$VARS{PROGRESS_REPORT}')\n?>\n";
    print OUTPUT "<div id='includedContent'>{{ progress_report|safe }}</div>\n";
...
if ($isServer == 1) {
	print LOG "Update_Progress(\"$VARS{WorkingDir}$VARS{PROGRESS_REPORT}\",\"scores\");\n";
	Update_Progress("$VARS{WorkingDir}$VARS{PROGRESS_REPORT}","scores");
}
...
my $MSA_LengthFile = "$VARS{WorkingDir}/MSA_LENGTH";
open MSA_LENGTH_OUT, ">$MSA_LengthFile";
	print MSA_LENGTH_OUT $VARS{MSA_LENGTH};
close MSA_LENGTH_OUT;
...
# JalView outputs for MSA without low col sp
$VARS{Alignment_File_without_low_SP_Col_with_Names_JalView}="$VARS{Alignment_File_without_low_SP_Col_with_Names}.Jalview.html";
Guidance::make_JalView_output_ONLY_MSA("$VARS{WorkingDir}$VARS{Alignment_File_without_low_SP_Col_with_Names_JalView}",$VARS{run_url},$VARS{Alignment_File_without_low_SP_Col_with_Names});
...
if ($FORM{PROGRAM} eq "GUIDANCE2")
{
    ...
    system ("cp -r $VARS{GUIDANCE2_MSAs_Dir}/wNames $VARS{WorkingDir}$VARS{Output_Prefix}_AlternativeMSA/");
    # list the default and alternative MSAs
    $VARS{List_Of_Alternative_MSAs}="$VARS{WorkingDir}List_Of_Default_and_AltMSAs.txt";
    system ("ls -1 $VARS{WorkingDir}$VARS{Alignment_File_With_Names} $VARS{WorkingDir}$VARS{Output_Prefix}_AlternativeMSA/*.fasta > $VARS{List_Of_Alternative_MSAs}");
}
...
#print_message_to_output("<A HREF='$VARS{Output_Prefix}_res_pair_seq_pair.scr' TARGET=results_res_pair_seq_pair>GUIDANCE residue pair sequence pair scores</A></font><br>");
	...
	if ($MEAN_RES_PAIR_SCORE<1) # print masking and filtering options
	{
		print_message_to_output("<form enctype=\"multipart/form-data\" action=\"/mask.php\" method=\"post\">Mask specific residues below a certain cutoff: <input type=\"hidden\" name=\"run_num\" value=\"$VARS{run_number}\"/> <input type=\"hidden\" name=\"VARS\" value=\"$stored_data_file\"/>  ".print_remove_site_selection_mask("$VARS{WorkingDir}$VARS{Output_Prefix}_res_pair_res.scr",$VARS{MSA_LENGTH})."<input type=\"hidden\" name=\"type_a\" value=\"$cmd1\"/> <input type=\"submit\" value=\"Mask Residues\"/> <font size =\"-1\">(see <a href=\"$overview_URL#Mask_RES\" target=\"_blank\">help</a></font>)</form></li>\n</ul>");
...
###print OUTPUT "</ul>\n";
		...
		if ((!-e "$VARS{WorkingDir}$VARS{removed_low_SP_SITE}") or ((-s "$VARS{WorkingDir}$VARS{removed_low_SP_SITE}") == 0)) # nothing to remove
		{
			print OUTPUT "<ul>";
			print_message_to_output(" All positions had score higher than ".sprintf("%.3f", $VARS{SP_COL_CUTOFF})."<br>");
			print OUTPUT "</ul>\n";
		}
		elsif (!-e "$VARS{WorkingDir}$VARS{Alignment_File_without_low_SP_Col_with_Names}") # filtered is empty file -> all positions were removed...
		{
			print OUTPUT "<ul>";
		...
		{
			print OUTPUT "<ul>";
			print_message_to_output("<A HREF='$VARS{Alignment_File_without_low_SP_Col_with_Names}' TARGET=_blank>The MSA after the removal of unreliable columns (below $VARS{SP_COL_CUTOFF})</A><font size=-1> (see list of removed columns <A HREF='$VARS{removed_low_SP_SITE}' TARGET=_blank>here</A>)</font>  - Open <A HREF='$VARS{JalView_Page}' TARGET=_blank>base</A> and <A HREF='$VARS{Alignment_File_without_low_SP_Col_with_Names_JalView}' TARGET=_blank>filtered</a> MSAs with JalView<br>");
###	print_message_to_output("<A HREF='$VARS{Alignment_File_without_low_SP_Col_with_Names}' TARGET=Alignmnet_without_low_SP>The MSA after the removal of unreliable columns (below ".sprintf("%.3f", $VARS{SP_COL_CUTOFF}).")</A><font size=-1> (see list of removed columns <A HREF='$VARS{removed_low_SP_SITE}' TARGET=_blank>here</A>)</font><br>"); 
		...
		my $MSA_DepthFile = "$VARS{WorkingDir}/MSA_DEPTH";
		open MSA_DEPTH_OUT, ">$MSA_DepthFile";
			print MSA_DEPTH_OUT $VARS{MSA_Depth};
		close MSA_DEPTH_OUT;
		...
		if (!-e "$VARS{WorkingDir}$VARS{Seq_File_without_low_SP_SEQ_with_Names}") # all positions were removed [default score]
		{
			print OUTPUT "<ul>";
			print_message_to_output("<font color='red'><B>ATTENTION:</font></B> All sequences had score below  $VARS{SP_SEQ_CUTOFF}<br>");
			print OUTPUT "</ul>";
		}
		elsif (!-e "$VARS{WorkingDir}$VARS{removed_low_SP_SEQ_With_Names}") # no positions were removed [default score]
		{
			print OUTPUT "<ul>";
	...
	{
		print_message_to_output ("According to the $FORM{PROGRAM} scores, the alignment is highly reliabile, no positions or sequences to mask or remove<br>");
	}
	if ($FORM{PROGRAM} eq "GUIDANCE2") #  TO DO -> make it available aso for other methods... needs to convert names back to original
	{
		print OUTPUT "<ul><li><form enctype=\"multipart/form-data\" action=\"/ConcatMSAs.php\" method=\"post\"> <font color=red face=\"Comic Sans MS\">New feature </font> - Create a <b>SuperMSA</b> by concatenating the base MSA and ".print_SuperMSA_selection($VARS{List_Of_Alternative_MSAs})." alternative MSAs computed by $FORM{PROGRAM} <input type=\"hidden\" name=\"run_num\" value=\"$VARS{run_number}\"/> <input type=\"submit\" value=\"Create SuperMSA\"/> <font size =\"-1\">(see <a href=\"$overview_URL#SuperMSA\" target=\"_blank\">help</a></font>)</form></li>\n</ul>\n\n<!-- SuperMSA_results_section -->\n\n";
		print_message_to_output("<A HREF='$VARS{Output_Prefix}_AlternativeMSA.tar.gz'> The set of alternative MSAs<br></A>");
	}	
	if ($FORM{Seq_Type} eq "Codons")
	{
		print_message_to_output("<A HREF='$home_URL/index_rerun_same_seq.php?run=$VARS{run_number}&file=$VARS{SeqsFile_Codons}'> Run GUIDANCE again using the same seqeunce file<br></A>");
	}
	else
	{
		print_message_to_output("<A HREF='$home_URL/index_rerun_same_seq.php?run=$VARS{run_number}&file=$VARS{SeqsFile}'> Run GUIDANCE again using the same seqeunce file<br></A>");
	}
...
#finish_output();
	...
	if ($FORM{JOB_TITLE} eq "daily_test") {
        $cmd = $VARS{write_daily_test_cmd};
        system ($cmd);
	}
	Prepare_rerun_param();
...
sub run_Guidance2
{
	...
	my $Single_MSA_Time="NA";
	my $Estimated_Alt_MSA_SecTime="NA";
	my $Estimated_Alt_MSA_Time_formatted_str="NA";
	if (exists $VARS{BASE_ALIGN_TIME_SEC})
	{
		$Estimated_Alt_MSA_SecTime=$FORM{Bootstraps}*$VARS{BASE_ALIGN_TIME_SEC};
	}
	if ($Estimated_Alt_MSA_SecTime ne "NA")
	{
		$Estimated_Alt_MSA_Time_formatted_str=Get_Formated_Time($Estimated_Alt_MSA_SecTime);
	}
...
#	my $proc_num = 8;
	...
	for (my $proc=0; $proc<$VARS{proc_num}; $proc++) {
		...
		if ($pid) {
			...
			for (my $tree_num=0; $tree_num<$bp_per_proc; $tree_num++) {
				...
				my $HOT_START_TIME=time();
				system ($cmd);
				my $HOT_END_TIME=time();
				if (!exists $VARS{BASE_ALIGN_TIME_SEC})
				{
					$VARS{BASE_ALIGN_TIME_SEC}=($HOT_END_TIME-$HOT_START_TIME);
				}
				if ($isServer==1)
				{
					my $i=$tree_num/$VARS{proc_num};
					my $Estimated_Time_left="";
					my $Estimated_Time_left_formated_str="";
					if (exists $VARS{BASE_ALIGN_TIME_SEC})
					{
						$Estimated_Alt_MSA_SecTime=($FORM{Bootstraps}-$i)*$VARS{BASE_ALIGN_TIME_SEC};
					}
					if ($Estimated_Alt_MSA_SecTime ne "NA")
					{
						$Estimated_Alt_MSA_Time_formatted_str=Get_Formated_Time($Estimated_Alt_MSA_SecTime);
					...
					open (PROGRESS,">$VARS{WorkingDir}$VARS{ALT_MSA_STATUS}");
					if ($VARS{NumOfSeq}==2)
					{
						#JS - removed estimated time
						#print PROGRESS "\n<ul class=\"in_progress\"><li>",($i+1)*2," out of ",$FORM{Bootstraps}*2," alternative alignments were created (estimated time till end: $Estimated_Alt_MSA_Time_formatted_str)</li></ul>\n";
						print PROGRESS "\n<ul class=\"in_progress\"><li>",($i+1)*2," out of ",$FORM{Bootstraps}*2," alternative alignments were created</li></ul>\n";
					}
					else
					{
						#JS - removed estimated time
						#print PROGRESS "\n<ul class=\"in_progress\"><li>",($i+1)*4," out of ",$FORM{Bootstraps}*4," alternative alignments were created (estimated time till end: $Estimated_Alt_MSA_Time_formatted_str)</li></ul>\n";
						print PROGRESS "\n<ul class=\"in_progress\"><li>",($i+1)*4," out of ",$FORM{Bootstraps}*4," alternative alignments were created</li></ul>\n";
					}
					close (PROGRESS);
					#send_http_request ($VARS{run_number})
				}
			}
			exit 0;
		} else {
	...
	if ($VARS{NumOfSeq}==2)
	{
		if ($aln_count<$Num_of_Aln_from_HoT_per_Run*$FORM{Bootstraps}/2) # ONLY HoT (no tree)
		{
	...
	if ($isServer == 1) {
		print "Update_Progress(\"$VARS{WorkingDir}$VARS{PROGRESS_REPORT}\",\"alternative\");\n";
		Update_Progress("$VARS{WorkingDir}$VARS{ALT_MSA_STATUS}","alternative");
	}
}
sub run_HoT
{
...
sub send_finish_email_to_user
{
	...
	my $HttpPath="http://guidance-dev.tau.ac.il/results/$VARS{run_number}";
	...
	my $msg = "cd $VARS{send_email_dir}; ".'perl ./sendEmail.pl -f \''.GENERAL_CONSTANTS::ADMIN_EMAIL.'\' -t \''.$FORM{user_mail}.'\' -u '.$email_subject.' -xu '.$VARS{userName}.' -xp '.$VARS{userPass}.' -s '.$VARS{smtp_server}.' -m '.$email_message;
...
#---------------------------------------------
{
	...
	print OUTPUT "&nbsp;|&nbsp;\n";
	print OUTPUT "<a href=\"$credits_URL\" target=\"_blank\">Credits</a>\n";
	print OUTPUT "</span>\n";
	print OUTPUT "<br />\n";
	print OUTPUT "</div>\n";
...
#	}
	...
	if ($isServer == 1) {
		Update_Progress("$VARS{WorkingDir}$VARS{PROGRESS_REPORT}","Constructing bootstrap");
	}
}
...
#	}
	...
	elsif ($FORM{MSA_Program} eq "PAGAN")
	{
		...
		# in newer version rough tree is not needed anymore. Consider to make a PAGAN version switch to support older version. For now, supports only the neweer...
...
#		$cmd=$pagan_prog." --seqfile $VARS{WorkingDir}$VARS{codded_seq_fileName} --treefile $VARS{WorkingDir}$RoughTree_MAFFT --outfile $VARS{WorkingDir}$VARS{Alignment_File}";
		$cmd=$pagan_prog." --seqfile $VARS{WorkingDir}$VARS{codded_seq_fileName} --outfile $VARS{WorkingDir}$VARS{Alignment_File}";
		print LOG "Align: $cmd\n";
		system ($cmd);
	...
	if ($isServer == 1) {
		Update_Progress("$VARS{WorkingDir}$VARS{PROGRESS_REPORT}","Generating the base alignment");
	}
	my $BASE_ALIGN_END_TIME=time();
	$VARS{BASE_ALIGN_TIME_SEC}=$BASE_ALIGN_END_TIME-$BASE_ALIGN_START_TIME+$OVERHEAD_TIME;
	### tests... ###
	my $formated_time_str=Get_Formated_Time($VARS{BASE_ALIGN_TIME_SEC});
	print "BASE MSA TIME:$VARS{BASE_ALIGN_TIME_SEC}\tRet:$formated_time_str\n";#<STDIN>;
}
...
#---------------------------------------------
	...
	if ($isServer == 0) {
   	...
   	if ($isServer == 1) {
		...
		print OUTPUT "<hr> <h4 class=footer><p align='center'>\nQuestions and comments are welcome! Please <span class=\"admin_link\"><a href=\"mailto:bioSequence\@tauex.tau.ac.il\?subject=GUIDANCE\%20Run\%20Number\%20$VARS{run_number}\">contact us</a></span></p></h4>\n<div id=\"bottom_links\"> <!-- links before the footer --><span class=\"bottom_link\"> <a href=\"$home_URL\" target=\"_blank\">Home</a> &nbsp;|&nbsp<a href=\"$overview_URL\" target=\"_blank\">Overview</a> &nbsp;|&nbsp;<a href=\"$gallery_URL\" target=\"_blank\">Gallery</a> &nbsp;|&nbsp;<a href=\"/$credits_URL\" target=\"_blank\">Credits</a> </span> <br /> </div>";
		print OUTPUT "</body>\n";
		print OUTPUT "</html>\n";	
		close OUTPUT;
		#if (($FORM{user_email} ne "") and ($FORM{user_email} ne "daily_test"))
		#{
		#	send_mail_on_error();
		#}
...
#		my $msg = "ssh bioseq\@lecs \"cd $VARS{send_email_dir};".'perl sendEmail.pl -f \''.GENERAL_CONSTANTS::ADMIN_EMAIL.'\' -t \''.$FORM{user_email}.'\' -u '.$email_subject.' -xu '.$VARS{userName}.' -xp '.$VARS{userPass}.' -s '.$VARS{smtp_server}.' -m '.$email_message."\""; # ACTIVATE IN CASE THE CLUSTER NODE FAILS TO COMMUNICATE WITH THE NET
        my $msg = 'perl ./sendEmail.pl -f \''.GENERAL_CONSTANTS::ADMIN_EMAIL.'\' -t \''.$FORM{user_email}.'\' -u '.$email_subject.' -xu '.$VARS{userName}.' -xp '.$VARS{userPass}.' -s '.$VARS{smtp_server}.' -m '.$email_message;
...
#		my $msg = "ssh bioseq\@lecs \" cd $VARS{send_email_dir};".'perl sendEmail.pl -f \'bioSequence@tauex.tau.ac.il\' -t \''."bioSequence\@tauex.tau.ac.il".'\' -u '.$email_subject.' -xu '.$VARS{userName}.' -xp '.$VARS{userPass}.' -s '.$VARS{smtp_server}.' -m '.$email_message."\""; # ACTIVATE IN CASE THE CLUSTER NODE FAILS TO COMMUNICATE WITH THE NET
		my $msg = "perl ".$VARS{send_email_dir}.'/sendEmail.pl -f \'bioSequence@tauex.tau.ac.il\' -t \''."bioSequence\@tauex.tau.ac.il".'\' -u '.$email_subject.' -xu '.$VARS{userName}.' -xp '.$VARS{userPass}.' -s '.$VARS{smtp_server}.' -m '.$email_message;
		#  print LOG "MESSAGE:$email_message\nCOMMAND:$msg\n";
        chdir $VARS{send_email_dir};
        my $email_system_return = `$msg`;
		$msg = "perl ".$VARS{send_email_dir}.'/sendEmail.pl -f \'bioSequence@tauex.tau.ac.il\' -t \''."haim.ashkenazy\@gmail.com".'\' -u '.$email_subject.' -xu '.$VARS{userName}.' -xp '.$VARS{userPass}.' -s '.$VARS{smtp_server}.' -m '.$email_message;
		`$msg`;
}
...
sub Prepare_rerun_param
{
	...
	else {print PARAM "\$Align_Order=1;\n";} #default
...
sub Print_Initial_Running_Progress
{
	$VARS{PROGRESS_REPORT}="ProgressReport.html";
	$VARS{ALT_MSA_STATUS}="MSA_STATUS.txt";
	open (ALT_STATUS,">>$VARS{WorkingDir}$VARS{ALT_MSA_STATUS}");
	print ALT_STATUS "<ul class=\"in_progress\"><li>Generating alternative alignments</li></ul>\n";
	close (ALT_STATUS);
	open (my $PROGRESS,">>","$VARS{WorkingDir}$VARS{PROGRESS_REPORT}");
	print $PROGRESS "<p><font face=Verdana size=2>\n";
	if ($FORM{Redirect_From_MAFFT} ne "1")
	{
		print $PROGRESS "<ul class=\"in_progress\"><li>Generating the base alignment</li></ul>\n";
	}
	print $PROGRESS "<ul class=\"in_progress\"><li>Constructing bootstrap guide-trees</li></ul>\n";
	print $PROGRESS "REPLACE";
    # comment out JS 
	#print $PROGRESS "<?php\n\tif (file_exists('$VARS{ALT_MSA_STATUS}'))\n\t{\n\t\t\$fil = fopen('$VARS{ALT_MSA_STATUS}', 'r');\n\t\t\$dat = fread(\$fil, filesize('$VARS{ALT_MSA_STATUS}'));\n\t\techo \"\$dat\";\n\tfclose(\$fil);\n\t}\n?>\n";
	if ($FORM{PROGRAM} eq "GUIDANCE")
	{
		print $PROGRESS "<ul class=\"in_progress\"><li>Calculating GUIDANCE scores</li></ul>\n";
	}	
	if ($FORM{PROGRAM} eq "HoT")
	{
		print $PROGRESS "<ul class=\"in_progress\"><li>Calculating HoT scores</li></ul>\n";
	}
	if ($FORM{PROGRAM} eq "GUIDANCE2")
	{
		print $PROGRESS "<ul class=\"in_progress\"><li>Calculating GUIDANCE2 scores</li></ul>\n";
	}
	if ($FORM{PROGRAM} eq "GUIDANCE3")
	{
		print $PROGRESS "<ul class=\"in_progress\"><li>Calculating GUIDANCE3 scores</li></ul>\n";
	}
	print $PROGRESS "</font>\n";
	close ($PROGRESS);
	#send_http_request ($VARS{run_number})
...
sub Update_Progress
{
	my $ProgressFile=shift;
	my $message=shift;
	
	open (my $PROGRESS,"<",$ProgressFile);
	my @data=<$PROGRESS>;
	close ($PROGRESS);
	...
	foreach my $line (@data)
	{
		if ($line=~/$message/)
		{
			$line=~s/in_progress/finished/;
			if ($line=~/\(?estimated time.*?\)?/) # remove estimated time message
			{
				$line=~s/\(estimated time.*?\)//;
			}
			print $PROGRESS $line;
...
sub Get_Formated_Time
{
	...
	if ($Time_in_Min<1)
	{
		$RetTime=$Time_in_Sec." seconds";
	}
	else
	{
		$RetTime=BIOSEQUENCE_FUNCTIONS::time_in_days_from_minutes($Time_in_Min);
...
sub Change_Qued_To_Running
{
	my $out_html=shift;
	print "Change_Qued_To_Running $out_html\n";
	open (my $OUT,"<",$out_html) || print "failed open file '$out_html' $!";
	my @file=<$OUT>;
	close ($OUT);
	open ($OUT,">",$out_html);
	print join ("\n",@file)."\n";
...
sub print_SuperMSA_selection
{
    my $AltMSA_list=shift;
    my $Default_number_of_alternative_MSA_to_concat=20;
    open (my $LIST,"<",$AltMSA_list) or exit_on_error('sys_error', "print_SuperMSA_selection: Can't open the list with AltMSA file: '$AltMSA_list' $!");
    my $return_str.="<select name=\"NumOfMSAs\" id=\"NumOfMSAs\">";
 
    my $AltMSA_to_Concat_including_baseMSA=0;
    while (my $line=<$LIST>)
    {
        $AltMSA_to_Concat_including_baseMSA++;
        my $AltMSA_to_Concat=$AltMSA_to_Concat_including_baseMSA-1;
        if ($AltMSA_to_Concat>0)
        {
            if ($AltMSA_to_Concat == $Default_number_of_alternative_MSA_to_concat)
            {
                $return_str.="<option selected=\"selected\" value=\"$AltMSA_to_Concat_including_baseMSA\">".$AltMSA_to_Concat."</option> ";
            }
            else
            {
                $return_str.="<option value=\"$AltMSA_to_Concat_including_baseMSA\">".$AltMSA_to_Concat."</option> ";
            }
        }
    }
    $return_str.="</select>";
...
{
	my $process_id = shift;
	my $url = 'http://guidance-dev.tau.ac.il/update_html_ext/'.$process_id;
	print "$url\n";
