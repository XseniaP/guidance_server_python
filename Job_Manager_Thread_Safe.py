...
class Job_Manager_Thread_Safe:
    ...
    def __calc_num_running_processes(self):
        ...
        logger.info('calculating running_processes:')
        for process_id in self.__processes_state_dict:
            #for job_prefix in self.jobs_prefixes_lst: JS = this causes the bug
            job_prefix = sc.MAIN_JOB_PREFIX
            #logger.info(f'process_id = {process_id}, prefix = {job_prefix}, state = {self.__processes_state_dict[process_id].get_job_state(job_prefix)}')
    ...
    def add_process(self, process_id: str, job_prefix, *args):
        ...
        -------
        """
        logger.info(f'process_id = {process_id}, job_prefix = {job_prefix}, args = {args}, running_processes = {self.__calc_num_running_processes()}')
        # don't put inside the mutex area - the funciton acquire the mutex too
