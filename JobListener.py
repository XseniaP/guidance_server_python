...
class PbsListener:
    ...
    def handle_job_state(self, new_job_state):
        ...
        if len(changed_jobs.index) == 0:
            return
        logger.info(f'changed_jobs_job_state:\n {changed_jobs}')
            ...
            for index, job_row in relevant_df.iterrows():
                ...
                try:
                    ...
                    if job_status == 'R':
                        ...
                        logger.warning(f'job_row:\ {job_row} weird behavior, status = {job_status}')
                        # JS causes bug
                        #self.job_prefix_to_function_mapping[job_prefix][WEIRD_BEHAVIOR_JOB_TO_CHECK](job_number)
    ...
    def get_server_job_stats(self):
        ...
        """
        result = subprocess.run(['/opt/pbs/bin/qstat', '-f', '@power9', '-u', 'bioseq'], stdout=subprocess.PIPE)
        ...
        #logger.info(f'job_prefixes = {self.job_prefixes}')
